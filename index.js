...
app.post('/webhook', async (req, res) => {
    ...
    if (signalState[symbolDateKey].hasOwnProperty(stateKey)) {
        signalState[symbolDateKey][stateKey] = { price, time, newSince: Date.now() };
        console.log(`✅ State updated for ${symbolDateKey}: ${stateKey} -> ${price}`);
    } else {
    ...
    try {
        ...
        if (data.Items) {
            ...
            data.Items.forEach(item => {
                // ** THIS IS THE FIX **
                // We now check if the item has the correct 'symbolDate' key before processing it.
                // This prevents the application from crashing if it finds old data.
                if (item.symbolDate && item.symbolDate.S && item.stateData && item.stateData.S) {
                    ...
                    console.warn("⚠️ Found a malformed item in DynamoDB, skipping:", item);
                }
            });
            console.log(`✅ Successfully loaded ${symbolOrder.length} valid historical symbol entries from DynamoDB.`);
        }
    } catch (dbError) {
...
server.listen(port, async () => {
    await loadDataFromDB(); // Load all historical data
